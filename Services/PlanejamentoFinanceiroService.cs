...
public class PlanejamentoFinanceiroService(ILancamentosRepository lancamentos, IDividasRepository dividas, IContribuicoesRepository contribuicoes, IObjetivosRepository objetivos, ICalculadoraObjetivoService calculadora) : IPlanejamentoFinanceiroService
{
    ...
    {
        var p = new PainelViewModel { Receitas = await lancamentos.LancamentosAsync("Receitas", usuarioId), Despesas = await lancamentos.LancamentosAsync("Despesas", usuarioId), Dividas = await dividas.DividasAsync(usuarioId), Contribuicoes = await contribuicoes.ContribuicoesAsync(usuarioId) };
        p.Objetivos = (await objetivos.ObjetivosAsync(usuarioId)).Select(o => new ObjetivoPlanejado(o, calculadora.Calcular(o))).ToList();
